...
1. [Crude Class](#crude-class)
    1. [Crude Instantiation](#instantiation)
    1. [Crude Controller](#crude-controller)



### <a name="crude-class">Crude Class</a>
...
>    * **optExpressApp** `Express=` Optionally you may define an instance of Express.
>
...
* `readOne(query)` Gets invoked on a single record request.
    * **query** `Object` An object containing query parameters for fetching the record. Will forcibly contain a query based on the provided `id` of the resource to ensure a single record is selected.
* `update(query, data)` Update a record.
    * **query** `Object` An object containing query parameters for the record to update. Will forcibly contain a query based on the provided `id` of the resource to ensure a single record is selected.
    * **data** `Object` An Object with key/value pairs representing the attributes and values to update with.
* `delete(query)` Gets invoked on a Delete operation.
    * **query** `Object` An object containing query parameters for the record to delete. Will forcibly contain a query based on the provided `id` of the resource to ensure a single record is selected.
* `count(query)` Expects a count of all the records based on the provided query.
    * **query** `Object` An object containing query parameters for the count operation.

Any value returned by the controller methods will be used as output by Crude to the requesting client. __To perform asynchronous operations a Promise should be returned__.
