 ...
 * @protected
 */
CrudCmd.prototype._handleCreate = Promise.method(function(req, res, document){
  var self = this;
  ...
  if (!__.isString(document[self.opts.urlField])) {
    error = new Error(errorMsg);
    error.type = '200';
    throw error;
  }

  self.addFlashSuccess(req, document);
  res.redirect(self.getBaseUrl(req) + '/' + document[self.opts.urlField]);
});
...
CrudCmd.prototype._readOne = Promise.method(function(req, res){
  ...
  var self = this;
  var entity = new this.Entity(req.user);
  return entity.readOne(query, function(err, doc){
    if (err) {
